...
**经验法则**：如果某个中转站的价格显著低于官方定价的 50%，很可能使用了逆向或号池渠道。
...
**所有通过中转站的请求内容对中转站运营者完全可见。** 这意味着：

- 你发送给 AI 的**代码片段**、**数据库 schema**、**内部文档**都可能被记录
- 你在 prompt 中包含的**环境变量**、**配置信息**可能被提取
- AI 编码助手（如 Cursor、Windsurf）会自动将**项目代码上下文**发送到 API

```python
# ❌ 危险：AI 编码助手可能自动发送以下内容
# .env 文件内容
DATABASE_URL=postgresql://admin:password@prod-db:5432/main
AWS_SECRET_ACCESS_KEY=wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY

# ❌ 危险：代码中硬编码的密钥
api_key = "sk-xxx-real-production-key"
```
...
```bash
# 1. 配置 .gitignore / .cursorignore / .claudeignore
# 确保敏感文件不被 AI 工具读取
echo ".env" >> .cursorignore
echo ".env.local" >> .cursorignore
...
中转服务本质是用安全性换取便利性和低价。在使用时，务必明确自己的风险边界——什么数据可以过中转，什么数据必须走官方直连。

### 面向个人使用

- **日常开发/学习**：中转服务是务实选择，注意选择有一定口碑的服务商
- **安全底线**：配好 ignore 文件，不要在使用中转时暴露任何生产环境密钥
